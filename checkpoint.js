...
function stringMasLarga(strings) {
 ...
 for (let index = 1; index < strings.length; index++) {
   if(strings[index].length > palabraMasLarga.length){
     palabraMasLarga = strings[index]
...
function crearClasePersona() {
  ...
  class Persona {
   ...
   //var identidad = new Persona("nombre",20,["deportes","cine","natacion","fiesta"]);

    addFriend(nombre, edad) {
      // el metodo addFriend recibe un string nombre y un entero edad y debe agregar un objeto:
      // { nombre: nombre, edad: edad} al arreglo de amigos de la persona.
      // no debe retornar nada.
    var amigo1 = { nombre: nombre , edad: edad}
    this.amigos.push(amigo1)
    }
    ...
    getFriends() {
      ...
      var amigo1 = amigo1.map(function(el){
        return el.nombre
      })
    }
    ...
    getPromedioEdad() {
      ...
      var suma = this.amigos.reduce(function(acc,curr)
      {
        return acc + curr.edad
      },0)
     var promedioDeEdad = suma / this.edad.length
